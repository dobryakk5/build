...
async def _process_upload(job_id: str) -> None:
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            # У нового файла сметы не должно оставаться старого ENIR-маппинга.
            await db.execute(
                delete(EnirEstimateMapping)
                .where(EnirEstimateMapping.project_id == job.project_id)
            )
            await db.execute(
                delete(EnirGroupMapping)
                .where(EnirGroupMapping.project_id == job.project_id)
            )
