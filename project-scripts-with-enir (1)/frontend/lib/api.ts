...
// ── ЕНИР: маппинг сметы → параграфы ─────────────────────────────────────────
export const enirMapping = {
  /** Текущее состояние маппинга проекта */
  state: (pid: string) =>
    request<any>(`/projects/${pid}/enir-mapping`),

  /** Маппинг одной группы задач */
  mapGroup: (pid: string, taskId: string) =>
    request<any>(`/projects/${pid}/enir-mapping/group/${taskId}`, { method: "POST" }),

  /** Маппинг всех верхних групп */
  mapAll: (pid: string) =>
    request<any>(`/projects/${pid}/enir-mapping/all`, { method: "POST" }),

  /** Подтвердить / изменить сборник для группы */
  confirmGroup: (pid: string, mappingId: number, collectionId?: number) =>
    request<any>(`/projects/${pid}/enir-mapping/group/${mappingId}/confirm`, {
      method: "PATCH",
      body: JSON.stringify({ collection_id: collectionId ?? null }),
    }),

  /** Подтвердить / изменить параграф для строки сметы */
  confirmEstimate: (pid: string, mappingId: number, body: {
    paragraph_id?: number;
    norm_row_id?: string;
    norm_row_hint?: string;
  }) =>
    request<any>(`/projects/${pid}/enir-mapping/estimate/${mappingId}/confirm`, {
      method: "PATCH",
...
export const enir = {
  ...
  paragraphs: (collectionId: number, q?: string) =>
    request<any[]>(`/enir/${collectionId}/paragraphs${q ? `?q=${encodeURIComponent(q)}` : ""}`),

  /** Полный параграф (нормы, состав работ, звено, примечания) */
  paragraph: (paragraphId: number) =>
    request<any>(`/enir/paragraph/${paragraphId}`),
