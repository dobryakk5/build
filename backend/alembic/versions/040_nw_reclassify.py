"""
040_nw_reclassify.py
Перепрогон классификатора (app.services.nw_classifier) после фикса правил:
  • OOS «мостовых» убран — это мощение, не мост
  • Новые OOS-маркеры: карбамид, понтон, галерея, дорожная разметка, светофор,
    сигнальные столбики, огнезащита текстиля
  • NW-059 (бурение скважин) сужено только до Сб.4 (не путать со свайным бурением)
  • NW-068 (стропила) сужено только до Сб.7/10/12 (не путать с ж/б ригелями)
  • NW-017 (опалубка/армирование) regex сужен до конструктивного армирования
  • Новое: окраска фасадов → NW-077 (раньше падала в общий малярный NW-031)
  • Новое: роллеты/жалюзи → NW-031
  • Новое: присоединения/врезки канализ./водопровод → NW-061/NW-060

Делает DELETE + перевставляет всё.
"""
...
revision = "040_nw_reclassify"
down_revision = "039_nw_fer_table_mapping"
branch_labels = None
depends_on = None
...
def _reclassify():
    """Полный пересчёт классификации. Используется upgrade'ом."""
    from app.services.nw_classifier import classify_table

    bind = op.get_bind()

    # 1. Section mappings → dict
    ...
    print(f"[040] reclassified {table_count} tables, inserted {len(rows_to_insert)} rows")
...
def downgrade():
    # Восстанавливаем «старую» классификацию через тот же reclassify —
    # фактически downgrade тут — это просто очистка с пустым повторным прогоном
    # на текущей версии классификатора. Если нужна именно историческая версия —
    # делать через `git checkout` старого nw_classifier.py.
    op.execute(f"DELETE FROM {FER_SCHEMA}.nw_fer_table_mapping")
    _reclassify()
