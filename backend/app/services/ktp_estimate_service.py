    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            raw_groups = await _run_stage1_ai(
                estimates, row_keys, wt_palette, kind_label, _progress, diagnostics
            )
...
async def _run_stage1_ai(
    ...
    if len(keys) <= chunk_size:
        ...
        prompt = _build_stage1_prompt(
            [(k, row_keys[k]) for k in keys], wt_palette, kind_label, gap_fill=True
        )
    ...
    for chunk_idx, start in enumerate(range(0, len(keys), chunk_size), start=1):
        ...
        prompt = _build_stage1_prompt(
            [(k, row_keys[k]) for k in chunk], wt_palette, kind_label, gap_fill=False
        )
    ...
    if successful_chunks > 0:
        ...
        gap_groups = await _run_global_gap_fill(groups, kind_label, order)
        groups.extend(gap_groups)
    return groups


async def _run_global_gap_fill(
    groups: list[dict[str, Any]], kind_label: str, order_offset: int
) -> list[dict[str, Any]]:
...
def _build_stage1_prompt(
    ...
    gap_instruction = (
        "5. Добавь технологически необходимые работы, которые ОТСУТСТВУЮТ в "
        ...
        if gap_fill
        else "5. НЕ добавляй работы, которых нет в списке позиций — только "
...
2. Внутри каждой группы укажи работы; нормализуй неаккуратные наименования.
3. Объедини дубли и однотипные позиции.
4. Для каждой работы из сметы укажи "origin": "from_estimate" и её "row_key".
{gap_instruction}
...
def _build_stage2_prompt(
    group: KtpWbsGroup, items: list[KtpWbsItem], answers: dict[str, str]
) -> str:
...
async def generate_card_for_wbs_group(
    ...
    prompt = _build_stage2_prompt(group, items, answers or {})
